...
class FlexibleReportTypeClassifier:
    def __init__(self, embedding_type: str = "keywords"):
        """
        Initialize classifier with choice of embedding type.
        embedding_type: "keywords" or "phrases"
        """
    ...
    def _precompute_embeddings(self):
        print(f"Precomputing {self.embedding_type} embeddings...")
        for report_data in self.report_data:
            report_type = report_data["report_type"]
            content = report_data[self.embedding_type]
    ...
    def find_best_report_type_v1(self, user_query: str) -> tuple[str, Dict]:
        """
        Calculate similarity with each keyword/phrase, then average per report type
        """
...
if __name__ == "__main__":
    ...
    print("=== FLEXIBLE REPORT TYPE CLASSIFICATION COMPARISON ===\n")
    
    # Test both keyword and phrase approaches
    embedding_types = ["keywords", "phrases"]
    methods = ["average", "max", "weighted"]
    
    results_comparison = {}
    
    for embedding_type in embedding_types:
        print(f"\n=== TESTING {embedding_type.upper()} APPROACH ===")
        classifier = FlexibleReportTypeClassifier(embedding_type=embedding_type)
        ...
        print(f"\n=== ACCURACY TESTING FOR {embedding_type.upper()} ===")
        ...
        results_comparison[embedding_type] = accuracy_results
        
        print("\n" + "="*80)
    
    # Final comparison
    print("\n=== FINAL COMPARISON: KEYWORDS vs PHRASES ===")
    print("-" * 60)
    total_queries = len(test_queries)
    
    for method in methods:
        keyword_accuracy = (results_comparison["keywords"][method] / total_queries) * 100
        phrase_accuracy = (results_comparison["phrases"][method] / total_queries) * 100
        
        print(f"{method.upper()} Method:")
        print(f"  Keywords: {results_comparison['keywords'][method]}/{total_queries} ({keyword_accuracy:.1f}%)")
        print(f"  Phrases:  {results_comparison['phrases'][method]}/{total_queries} ({phrase_accuracy:.1f}%)")
        
        better = "Phrases" if phrase_accuracy > keyword_accuracy else "Keywords"
        if phrase_accuracy == keyword_accuracy:
            better = "Tie"
    ...
    comparison_file = "./extensions/keywords_vs_phrases_comparison.txt"
    with open(comparison_file, 'w') as f:
        f.write("=== KEYWORDS vs PHRASES CLASSIFICATION COMPARISON ===\n\n")
        
        f.write("SUMMARY:\n")
        f.write("-" * 40 + "\n")
        for method in methods:
            keyword_acc = (results_comparison["keywords"][method] / total_queries) * 100
            phrase_acc = (results_comparison["phrases"][method] / total_queries) * 100
            f.write(f"{method.upper()} Method:\n")
            f.write(f"  Keywords: {results_comparison['keywords'][method]}/{total_queries} ({keyword_acc:.1f}%)\n")
            f.write(f"  Phrases:  {results_comparison['phrases'][method]}/{total_queries} ({phrase_acc:.1f}%)\n\n")
        
        f.write("\nDETAILED MISCLASSIFICATIONS:\n")
        f.write("=" * 50 + "\n")
        
        for embedding_type in embedding_types:
            f.write(f"\n{embedding_type.upper()} MISCLASSIFICATIONS:\n")
            f.write("-" * 30 + "\n")
            ...
            for query, expected_type in test_queries:
                ...
                if predicted != expected_type:
                    ...
                    f.write(f"Expected: {expected_type} | Predicted: {predicted}\n")
                    
                    # Get detailed scores for analysis
                    analysis = classifier.analyze_query(query)
                    weighted_scores = analysis['weighted']['detailed_scores']
                    
                    # Find top 3 predictions
                    all_scores = [(rt, data['combined_score']) for rt, data in weighted_scores.items()]
                    sorted_scores = sorted(all_scores, key=lambda x: x[1], reverse=True)[:3]
                    
                    f.write("Top 3 Predictions:\n")
                    for i, (rt, score) in enumerate(sorted_scores):
                        f.write(f"  {i+1}. {rt}: {score:.4f}\n")
                    
                    # Show score for expected type
                    expected_score = weighted_scores[expected_type]['combined_score']
                    f.write(f"Expected type score: {expected_score:.4f} (Rank: {sorted_scores.index((expected_type, expected_score)) + 1 if expected_type in dict(sorted_scores) else 'N/A'})\n")
                    
                    # Show best keyword match
                    best_match = weighted_scores[predicted]['best_keyword']
                    f.write(f"Best match in predicted: '{best_match}'\n")
                    
                    f.write("\n" + "-"*80 + "\n\n")
            
            f.write(f"Total misclassifications for {embedding_type}: {misclass_count}/{total_queries}\n\n")
